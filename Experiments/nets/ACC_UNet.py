...
class ChannelSELayer(torch.nn.Module):
    ...
    def __init__(self, num_channels):
        ...
        # 用于二维输入的自适应平均池化层，将输入的特定层自适应的输出为大小为1*1的张量
...
class HANCLayer(torch.nn.Module):
    ...
    def forward(self, inp):
        ...
        if self.k == 1:
            ...
            x = torch.concat(# 沿着指定的维度拼接起来
                [
                    x,
                    torch.nn.Upsample(scale_factor=2)(torch.nn.AvgPool2d(2)(x)),# 保留平均池化的信息，但是保持空间维度不变
...
class HANCBlock(torch.nn.Module):
    ...
    def __init__(self, n_filts, out_channels, k=3, inv_fctr=3):
        ...
        self.norm1 = torch.nn.BatchNorm2d(n_filts * inv_fctr)# n_filts * inv_fctr计算批归一化需要处理的特征数量
        ...
        self.conv2 = torch.nn.Conv2d(
            ...
            kernel_size=3,
            padding=1,
            # 如果设置为一说明所有输入的通道会被用于所有的输出通道的计算，也就是每个输出通道是所有输入的线性组合
            # 如果＞1说明输出的只是与一组的输入有关
            # 设置为n_filts * inv_fctr说明每个输入通道独立卷积，也就是与自己卷积，不会和其他通道混合，也就是深度可分离卷积
...
class ResPath(torch.nn.Module):
    ...
    def __init__(self, in_chnls, n_lvl):
        ...
        self.convs = torch.nn.ModuleList([])# 用来保存神经网络模块的列表容器
...
class ACC_UNet(torch.nn.Module):
    ...
    def __init__(self, n_channels, n_classes, n_filts=32):
        ...
        if n_classes == 1:
            ...
            self.last_activation = torch.nn.Sigmoid()# sigmoid将任意值压缩到区间0和1之间，用于二分类的输出层 
        else:
