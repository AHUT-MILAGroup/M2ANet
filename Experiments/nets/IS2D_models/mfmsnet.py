...
from nets.IS2D_models import load_cnn_backbone_model

def get_freq_indices(method):
    # 根据给定的方法，返回一组频率索引，用来处理图像或者信号数据，将数据映射到特定的频率分量
    assert method in ['top1','top2','top4','top8','top16','top32',
                      'bot1','bot2','bot4','bot8','bot16','bot32',
                      'low1','low2','low4','low8','low16','low32']
    num_freq = int(method[3:])# 提取数字
    if 'top' in method:
...
class MultiFrequencyChannelAttention(nn.Module):
    def __init__(self,
        ...
        for freq_idx in range(frequency_branches):
            # 为每个频率分支注册一个缓冲区，这个缓冲区存储的是DCT（离散余弦变换）滤波器权重
            # dct_h 和 dct_w 指定了DCT滤波器的高度和宽度。
            # mapper_x 和 mapper_y 是索引数组，它们指定了您想要在每个频率分支中关注的频率位置。
            # in_channels 指的是输入图像的通道数
    ...
    def forward(self, x):
        ...
        # self.state_dict()：调用模型实例的state_dict()方法，返回一个包含模型所有参数的字典。字典的键是参数的名称（字符串形式），值是对应的参数（通常是torch.Tensor对象）。
        # .items()：这是Python字典的一个方法，它返回一个包含字典中所有(键, 值)对的视图对象
    ...
    def build_filter(self, pos, freq, POS):
        # 用来计算DCT滤波器在给定位置上的系数
        # pos：当前的位置索引（在x或y维度上）。
        # freq：频率索引，指定了要关注的频率位置。
        # POS：滤波器在当前维度（x或y）上的大小。

        # 计算DCT的基础系数
        result = math.cos(math.pi * freq * (pos + 0.5) / POS) / math.sqrt(POS)
        # 处理直流分量，也就是freq==0，这个值表示了图像的平均亮度，对于DC分量，通常不进行缩放
        if freq == 0:
            return result
        # 对于交流分量AC，则进行缩放，保证DCT交换的正交性
        else:
...
class MFMSAttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        for scale_idx in range(self.scale_branches):
            ...
            feature = self.multi_scale_branches[scale_idx](feature)# 普通卷积
...
class UpsampleBlock(nn.Module):
    def __init__(self,
        ...
        self.conv1 = nn.Sequential(# 因为这边需要concat
...
class MFMSNet(nn.Module):
    ...
    def forward(self, x, mode='train1'):
