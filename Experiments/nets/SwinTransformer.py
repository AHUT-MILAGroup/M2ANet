...
def SwinB(pretrained=True):
    ...
    # embed_dim=128 指定了嵌入向量的维度。  
    # depths=[2, 2, 18, 2] 指定了每个阶段的Transformer块的数量。  
    # num_heads=[4, 8, 16, 32] 指定了每个阶段多头注意力机制中头的数量。  
    # window_size=12 指定了自注意力机制中窗口的大小。 
    model = SwinTransformer(embed_dim=128, depths=[2, 2, 18, 2], num_heads=[4, 8, 16, 32], window_size=12)
    if pretrained is True:# 添加预训练权重
        model.load_state_dict(torch.load('cch/ACC-UNet/Experiments/pretrained_ckpt/swin_base_patch4_window12_384_22kto1k.pth', map_location='cpu')['model'], strict=False)
...
if __name__ == '__main__':
    ...
    i=0
    for name,parad in model.named_parameters():
        i+=1
        print(i,":",name)