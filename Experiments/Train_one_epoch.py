...
def train_one_epoch(loader, model, criterion, optimizer, writer, epoch, lr_scheduler, model_type, logger):
    ...
    dices = []
    for i, (sampled_batch, names) in enumerate(loader, 1):# 表示索引从1开始
        ...
        # 如果是在验证阶段并且相隔一定的轮次进行可视化，每100次可视化一次
        ...
        if i == len(loader):# 最后一个批次的数量可能不是batch_size
    ...
    writer.add_scalar('epoch_' + logging_mode + '_' + loss_name, average_loss, epoch)# 损失曲线

    # plot metrics in tensorboard
    writer.add_scalar('epoch_' + logging_mode + '_iou', train_iou_average, epoch)# IOU曲线
    # writer.add_scalar(logging_mode + '_acc', train_acc, step)
    writer.add_scalar('epoch_' + logging_mode + '_dice', train_dice_avg, epoch)# Dice曲线
